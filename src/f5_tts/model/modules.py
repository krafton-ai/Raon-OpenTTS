...
def crop_waveform_to_hop_aligned_length(waveform, n_fft=1024, hop_length=256):
    """
    Crops waveform tail so that mel frames align exactly (assumes center=False).
    """
...
def get_sb_hifigan_mel_spectrogram(
    ...
    """
    Calculates clean MelSpectrogram with center=False and explicit reflect padding.
    """
    # 1. Apply manual reflect padding
...
class MelSpec(nn.Module):
    def __init__(
        ...
        super().__init__()
        if mel_spec_type not in ["vocos", "bigvgan", "sbhifigan16k"]:
            raise ValueError(f"Unsupported mel_spec_type '{mel_spec_type}'. Must be one of: vocos, bigvgan, sbhifigan16k")
...
class AttnProcessor:
    def __init__(
        ...
        logit_softcapping: float | None = None,  # softcap value for attention logits, e.g. 50.0
    ):
        ...
        if logit_softcapping is not None and attn_backend == "flash_attn":
            import warnings
            warnings.warn(
                "logit_softcapping is incompatible with flash_attn backend. "
                "Falling back to torch backend for softcapping support.",
                stacklevel=2,
            )
            attn_backend = "torch"
    ...
    def __call__(
        ...
        if self.attn_backend == "torch":
            ...
            if self.logit_softcapping is not None:
                # manual attention with logit softcapping (incompatible with SDPA)
